...
| 2-5 本番移行 | 🔄 進行中 | 手動入力データ完了、レインズデータ残り |
...
#### bkdata dataSource 調査結果（2026-01-11）
...
**エラーリトライ（2026-01-13）**:
- `003_1.JPG` (bkID 4492): 既にGCSに存在（84.1 KB）
- `千種区東山元町2丁目 土地...pdf` (bkID 53022): 新規アップロード成功（730.7 KB）

#### 残り作業: レインズデータ（2023年より前）

| 項目 | 値 |
|------|------|
| 件数 | 約29,000件 |
| 推定サイズ | 約15〜17 GB |
| 推定所要時間 | 約50〜70時間（HTTP経由） |
...
1. **TODO-02**: Blobstore → GCS 移行
   - ✅ 手動入力データ（7,421件）移行完了
   - **次の作業**: レインズデータ（約29,000件）の移行
   - コマンド: `python tools/download_and_upload_blob.py --to 2023/01/01`
