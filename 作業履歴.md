...
## 2026-01-13: TODO-02 手動入力データ GCSコピー完了

### 実施内容

#### 1. 手動入力データ（dataSource=null）の移行

bkdataの`dataSource`が空のデータ（手動入力されたデータ）をGCSにコピー。
...
PYTHONIOENCODING=utf-8 /c/Python311/python.exe tools/download_and_upload_blob.py --manual-only
```
...
SSL handshake failureで失敗した2件をリトライ。

##### リトライツール作成
- `migration-src/tools/retry_failed_blobs.py` を新規作成

##### リトライ結果
| bkID | ファイル名 | 結果 |
|------|-----------|------|
| 4492 | `003_1.JPG` | 既にGCSに存在（84.1 KB）※前回成功済み |
| 53022 | `千種区東山元町2丁目 土地（三井不動産リアルティ一社センター）.pdf` | 新規アップロード成功（730.7 KB） |

**最終結果**: 手動入力データ 7,462件中 7,423件（99.5%）がGCSにコピー完了

### GCS累計

| 区分 | 件数 | サイズ |
|------|------|--------|
| 2023/01/01 以降 | 264件 | 155.4 MB |
| 手動入力データ | 7,423件 | 1.60 GB |
| **累計** | **7,687件** | **約1.76 GB** |
...
```bash
cd /c/Users/hrsuk/prj/s-style-hrd/migration-src
PYTHONIOENCODING=utf-8 /c/Python311/python.exe tools/download_and_upload_blob.py --to 2023/01/01
```

### 注意事項
- Datastoreは**未更新**（blobKeyは元のまま）
- GCSへのファイルコピーのみ完了
- 本番移行時に `migrate_blob_to_gcs.py` でDatastore更新が必要
