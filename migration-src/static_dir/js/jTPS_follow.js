...
//		perPages:			[5, 10, 20, 50,'ALL'],				// the "show per page" selection
			perPages:			['ALL'],				// the "show per page" selection
...
//			$('>tbody>tr:last', this).after( '<tr class="stubCell"><td colspan="' + cols + '" style="height: ' + stubHeight + ';">&nbsp;</td></tr>' );
		...
		drawPageSelectors( this, page || 1,  $(this).data('tableSettings').targetTable );
...
//							$('>tbody>tr:last', pT).after( '<tr class="stubCell"><td colspan="' + cols + '" style="height: ' + stubHeight + ';">&nbsp;</td></tr>' );
						...
						if ($(this).data('tableSettings')) {
							drawPageSelectors( pT, 1,  $(this).data('tableSettings').targetTable );
						}
						else {
		...
		function drawPageSelectors ( target, page, target_table ) {
			...
			while ( pages-- )
				if (target_table != "") {
					$('>.nav .pagination', target).prepend( '<div id="' + target_table + ( pages + 1 ) + '" class="pageSelector">' + ( pages + 1 ) + '</div>' );
				}
				else {
...
//							var beginPos = ((page - 1) * perPage),
//								endPos = ( ( parseInt( $(this).html() ) - 1 ) * perPage ) - 1;
							var beginPos = ((page - 1) * perPage),
								endPos = ( ( parseInt( $(this).html() ) - 1 ) * perPage );
							if ( endPos > rowCount )
								endPos = (rowCount - 1);
...
//										$('>.nav .status', pT).html( 'Showing ' + ( cPos + 1 ) + ' - ' + ( cPos + ePos ) + ' of ' + rowCount + '' );
							...
							// redraw the pagination
							drawPageSelectors( pT, parseInt( $(this).html() ), target_table);
