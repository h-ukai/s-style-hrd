...
# Python 3: Google App Engine SDK (db) から google.cloud.ndb に移行
# db.Model → ndb.Model
# db.GqlQuery() → ndb.query() またはモデルクラスのquery()メソッド
...
class bkdataProvider():
    ...
    def search(self,bkID=None):
        if bkID:
            # REVIEW-L1: マイグレーションルール適用漏れ: Python 3では u"" は不要（すべて str）
            # 修正前: key_name = self.corp_name + u"/" + self.branch_name + u"/" + bkID
            # 修正後: key_name = self.corp_name + "/" + self.branch_name + "/" + bkID
            key_name = self.corp_name + "/" + self.branch_name + "/" + bkID
            # Python 3: db.Model.get_by_key_name() → ndb.Key().get()
            # REVIEW-L2: 例外処理の不足: 全例外をキャッチするのは推奨されない
            # 推奨: 特定の例外のみをキャッチ（例: except (AttributeError, db.BadKeyError) as e:）
            try:
            ...
            if bkd:
                ...
                # Python 3: db.GqlQuery() → モデルクラスのquery()メソッド
                # NOTE: 旧来の GQL 文字列ベースのクエリは ndb では直接サポートされていません
                # 以下はプレースホルダー実装です。実際の Blob モデルに合わせて修正してください
                # REVIEW-L2: 例外処理の不足: 全例外をキャッチするのは推奨されない
                # 推奨: ImportError と AttributeError を個別にキャッチ
                try:
                    from application.models import blob
                    self.blobs = blob.Blob.query(
                        blob.Blob.CorpOrg_key == self.corp_name,
                        blob.Blob.Branch_Key == self.branch_name,
                        blob.Blob.bkID == bkID
                    ).order_by(blob.Blob.media, blob.Blob.pos).fetch()
                except:
                    # Blob モデルがまだ ndb に対応していない場合のフォールバック
                    self.blobs = []
    ...
    def set(self,bkID=None):
        if bkID:
            ...
            # 修正後: key_name = self.corp_name + "/" + self.branch_name + "/" + bkID
            key_name = self.corp_name + "/" + self.branch_name + "/" + bkID
            # Python 3: db.Model.get_or_insert() → ndb.Model.get_or_insert()
            # REVIEW-L2: 例外処理の不足: 全例外をキャッチするのは推奨されない
            # 推奨: 特定の例外のみをキャッチ
            try:
                self.bkdb = bkdata.BKdata.get_or_insert(key_name, bkID=bkID)
                self.bkdb = timemanager.utc2jst_gql(self.bkdb)
            except:
                self.bkdb = None
    ...
    def put(self,req):
       ...
       #kkkksyk = db.BooleanProperty(verbose_name=u"価格交渉可")
        ...
        if knckJyukn:
            ...
            # 修正前: bkdb.knckJyukn = u"無"
            # 修正後: bkdb.knckJyukn = "無"
            bkdb.knckJyukn = "無"
        #オーナーチェンジ
...
#
# 2. モデル操作修正:
#    - db.Model.get_by_key_name() → ndb.Key(Model, key_name).get()
#    - db.Model.get_or_insert() → ndb.Model.get_or_insert()
#    - db.GqlQuery(query_str) → Model.query().filter(...).fetch()
#
# 3. 残りの Python 2→3 構文修正が必要:
#    - dict.iteritems() → dict.items()
#    - dict.itervalues() → dict.values()
#    - dict.iterkeys() → dict.keys()
#    - unicode 文字列リテラル (u'...') はそのまま有効（Python 3.3+）
#
# 4. 注意事項:
#    - Blob モデルへのクエリが example code になっているため、
#      実際の blob モデルに合わせて修正が必要です
#    - timemanager.utc2jst_gql() が ndb モデルに対応しているか確認してください
#    - request.get() は Flask の request.form.get() に移行の可能性あり
