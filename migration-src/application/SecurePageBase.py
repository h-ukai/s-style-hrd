...
class SecurePageBase:
    ...
    def Secure_init(self, *status_list, **kwargs):
        ...
        self.corp_name = self.pathParts[2] if len(self.pathParts) > 2 else ""
        self.branch_name = self.pathParts[3] if len(self.pathParts) > 3 else ""
        self.Sitename = self.pathParts[4] if len(self.pathParts) > 4 else ""
        ...
        ssn = dbsession(request, None, self.corp_name + "_" + self.branch_name + "_" + self.Sitename)
