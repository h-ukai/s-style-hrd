...
class GeoModel(ndb.Model):
  ...
  Attributes:
    location: A ndb.GeoPtProperty that defines the single geographic point
  ...
  def bounding_box_fetch(query, bbox, max_results=1000,
    ...
    Args:
      query: An ndb.Query on entities of this kind that should be additionally
    ...
    Raises:
      Any exceptions that ndb.Query.fetch() can raise.
    """
    ...
    if query_geocells:
      if query._query_order:
        ...
        cell_results = [copy.deepcopy(query)
            .filter(GeoModel.location_geocells == search_cell)
            .fetch(max_results) for search_cell in query_geocells]

        # Manual in-memory sort on the query's defined ordering.
        query_orderings = query._query_order or []
        def _ordering_fn(ent1, ent2):
          for prop, direction in query_orderings:
            prop_val1 = getattr(ent1, prop)
            prop_val2 = getattr(ent2, prop)
            if prop_val1 < prop_val2:
              prop_cmp = -1
            elif prop_val1 > prop_val2:
              prop_cmp = 1
            else:
              prop_cmp = 0
        ...
        # search.
        results = list(query
            .filter(GeoModel.location_geocells.IN_(query_geocells))
  ...
  def proximity_fetch(query, center, max_results=10, max_distance=0):
    ...
    Args:
      query: An ndb.Query on entities of this kind.
    ...
    def _merge_results_in_place(a, b):
      util.merge_in_place(a, b,
                        cmp_fn=lambda x, y: -1 if x[1] < y[1] else (1 if x[1] > y[1] else 0),
    ...
    while cur_geocells:
      ...
      temp_query = temp_query.filter(GeoModel.location_geocells.IN_(cur_geocells_unique))
      ...
      new_results = sorted(new_results, key=cmp_to_key(lambda dr1, dr2: -1 if dr1[1] < dr2[1] else (1 if dr1[1] > dr2[1] else 0)))
